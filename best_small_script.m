...
for r = runs
    ...
    T      = 2e4;         % No. of iterations (10K x 1-batch = 10K examples passed through the network)
    mu     = single(0.97);   % Momentum variable
    lambda = single(5e-4);   % Regularization constant
    eta    = @(t)((t > 7500)*1e-4 + (t <= 7500)*(5e-4)); % Learning rate - eta(t) returns the value for iteration t
