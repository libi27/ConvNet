...
eta    = @(t)(5e-3); % Learning rate - eta(t) returns the value for iteration t
...
train_acc_0 = good_pred/(good_pred+bad_pred);
fprintf('Train accuracy small network: %g\n',train_acc_0);
results_0(1,r) = train_acc_0;
% Test accuracy
...
test_acc_0 = good_pred/(good_pred+bad_pred);
fprintf('Test accuracy small network: %g\n',test_acc_0);
results_0(2,r) = test_acc_0;
...
net_arch = {...
    ...
    struct('type','relu','inInd',3,'outInd',3),  ...    
    struct('type','maxpool','inInd',3,'outInd',4,'kernelsize',pool_kernel,'stride',pool_stride), ...
    struct('type','conv','inInd',4,'outInd',5,'kernelsize',conv_kernel,'stride',conv_stride,'nOutChannels',conv_channels_1,'bias_filler',0),...
    ...
    struct('type','affine','inInd',6,'outInd',7,'nOutChannels',k_1,'bias_filler',0), ...
...
train_acc_1 = good_pred/(good_pred+bad_pred);
fprintf('Train accuracy big network: %g\n',train_acc_1);
results_1(1,r) = train_acc_1;
% Test accuracy
  % Construct test network (same as original, with image and label sources replaced)
test_net_arch          = net_arch;
test_net_arch{1}.fName = test_images_file;
...
test_acc_1 = good_pred/(good_pred+bad_pred);
fprintf('Test accuracy big network: %g\n',test_acc_1);
results_1(2,r) = test_acc_1;
end
results_0
results_1
